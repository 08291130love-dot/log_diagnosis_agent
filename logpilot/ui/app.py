...
from logpilot.config.settings import (
    DEFAULT_BASE_URL,
    DEFAULT_CHAT_MODEL,
...
from logpilot.rag.documents import read_uploaded_documents


from logpilot.config.paths import PROJECT_ROOT as ROOT, KNOWLEDGE_DIR, EVALUATION_CASES
...
def friendly_diagnosis_error(exc: Exception, api_key: str) -> str:
    from logpilot.integrations.mcp.client import MCPConnectionError
    ...
    if page == "仪表盘":
            ...
            if not api_key.strip():
                ...
                try:
                    ...
                    report = {
                        ...
                        "tool_transport": result.get("tool_transport", "local"),
                        "mcp": result.get("mcp", {}),
                        "steps": [
                            {
                                "tool": action.tool,
        ...
        if report and report.get("source_hash") == source_hash:
            ...
            with st.expander(f"查看调查过程（{len(report['steps'])} 次工具调用）"):
                if report.get("tool_transport") == "mcp":
                    mcp_info = report.get("mcp", {})
                    st.caption(f"工具通过 MCP 调用 · 本次请求 {mcp_info.get('request_id', '')[:8]} · {mcp_info.get('tool_calls', 0)} 次调用")

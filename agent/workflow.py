...
from agent.tools.source_tools import build_source_tools
...
def run_multi_diagnosis(repository, question, api_key, base_url, model,
                        ...
                        chat_history=None, embedding_model="text-embedding-v4"):
    ...
    logs = run_specialist("日志分析", "log_specialist.txt", build_log_tools(repository),
    ...
    if not logs.evidence:
        ...
        with ThreadPoolExecutor(max_workers=2, thread_name_prefix="diagnosis") as pool:
            source_tools = build_source_tools(source_repository)
            if source_tools:
            ...
            try:
                knowledge_tools = build_knowledge_tools(knowledge_base, api_key, base_url, embedding_model)
                if knowledge_tools:

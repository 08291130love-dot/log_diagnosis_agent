"""Opt-in real model comparison: python -m scripts.compare_agents --mode both."""
import argparse
import json
import os
...
from logpilot.config.settings import DEFAULT_BASE_URL, DEFAULT_CHAT_MODEL, DEFAULT_EMBEDDING_MODEL
...
from logpilot.config.paths import PROJECT_ROOT as ROOT, KNOWLEDGE_DIR, EVALUATION_RESULTS_DIR
...
def main():
    ...
    parser.add_argument("--transport", choices=["mcp", "local"], default="mcp", help="两种诊断模式使用相同的工具通信方式")
    ...
    knowledge = KnowledgeBase(KNOWLEDGE_DIR) if args.knowledge else None
    output_dir = EVALUATION_RESULTS_DIR / datetime.now().strftime("%Y%m%d-%H%M%S-%f")
    ...
    for mode in (["single", "multi"] if args.mode == "both" else [args.mode]):
        result = run_diagnosis(
            ...
            tool_transport=args.transport,
        )
        (output_dir / f"{mode}.md").write_text(result["output"], encoding="utf-8")
        row = {"mode": mode, "tool_transport": result["tool_transport"], "status": result.get("status", "completed"),
